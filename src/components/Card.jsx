...
const Card = ({ website, username, password, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-neutral-900 p-5 rounded-xl shadow hover:shadow-purple-600/20 transition border border-neutral-800 m-2 max-w-full overflow-hidden">
      <div className="flex justify-between items-center mb-4">
        {isEditing ? (
          <input
            type="text"
            className="bg-neutral-800 p-1 rounded text-white w-2/3 max-w-full"
            value={editData.website}
            onChange={(e) => setEditData({ ...editData, website: e.target.value })}
          />
        ) : (
          <h2 className="text-lg font-semibold text-purple-400 truncate max-w-[220px]">{website}</h2>
        )}
        <div className="flex gap-2">
          ...
          {confirmDelete && (
            <>
              ...
              <button onClick={() => setConfirmDelete(false)} className="hover:bg-white/10 p-1 rounded">
      ...
      <div className="flex justify-between mb-2">
        <div className="max-w-[220px] overflow-hidden">
          ...
          {isEditing ? (
            <input
              type="text"
              className="bg-neutral-800 p-1 rounded text-white w-full"
              value={editData.username}
              onChange={(e) => setEditData({ ...editData, username: e.target.value })}
            />
          ) : (
            <span className="text-white truncate inline-block align-middle">{username}</span>
          )}
        </div>
        {!isEditing && (
      ...
      <div className="flex justify-between items-center">
        <div className="max-w-[220px] overflow-hidden">
          ...
          {isEditing ? (
            ...
            <span className="text-white break-all inline-block align-middle">
